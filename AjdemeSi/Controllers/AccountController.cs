...
namespace AjdemeSi.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> LoginAsync(string email, string password, string returnUrl)
        {
            // This doesn't count login failures towards account lockout
            // To enable password failures to trigger account lockout, change to shouldLockout: true
            var result = await SignInManager.PasswordSignInAsync(email, password, true, shouldLockout: false);
            var res = SignInManager.PasswordSignIn(email, password, true, false);
            switch (result)
            {
                case SignInStatus.Success:
                    return Json(new { isValid = true, error = "" }, JsonRequestBehavior.AllowGet);
                case SignInStatus.LockedOut:
                    return View("Lockout");
                case SignInStatus.RequiresVerification:
                ...
                case SignInStatus.Failure:
                default:
                    return Json(new { isValid = false, error = "Invalid login attempt." }, JsonRequestBehavior.AllowGet);
            }
        }
        ...
        public async Task<ActionResult> RegisterAsync(string email, string password, bool isDriver)
        {
            ...
            // If we got this far, something failed, redisplay form
            return Json(new { isValid = false, error = "Invalid register attempt. " + (result.Errors.Any() ? String.Join("; ", result.Errors) : String.Empty) }, JsonRequestBehavior.AllowGet);
        }
